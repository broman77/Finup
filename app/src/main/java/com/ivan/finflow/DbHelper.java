...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public static class AccountItem {
        ...
        AccountItem(String name, double openingBalance) {
            this.name = name;
    ...
    public List<AccountItem> getAccountItems() {
        ...
        Cursor c = getReadableDatabase().query("accounts", new String[]{"name", "opening_balance"}, null, null, null, null, "id ASC");
        while (c.moveToNext()) out.add(new AccountItem(c.getString(0), c.getDouble(1)));
        c.close();
        return out;
    }

    public void ensureCategory(String type, String name) {
        if (name == null) return;
        ...
        int next = c.moveToFirst() ? c.getInt(0) : 0;
        c.close();
        insertCategory(db, type, cleaned, next);
    }
    ...
    public void replaceAllData(List<AccountItem> accounts, List<Tx> transactions, List<String> expenseCategories, List<String> incomeCategories) {
        SQLiteDatabase db = getWritableDatabase();
        db.beginTransaction();
        try {
            db.delete("tx", null, null);
            ...
            if (accounts != null) {
                for (AccountItem item : accounts) {
                    ...
                    cv.put("opening_balance", item.openingBalance);
                    db.insert("accounts", null, cv);
                }
            }
            ...
            if (expenseCategories != null) {
                ...
                for (String name : expenseCategories) {
                    if (name != null && !name.trim().isEmpty()) {
                        insertCategory(db, "expense", name.trim(), order++);
                        seeded = true;
                    }
                }
            }
            if (incomeCategories != null) {
                ...
                for (String name : incomeCategories) {
                    if (name != null && !name.trim().isEmpty()) {
                        insertCategory(db, "income", name.trim(), order++);
                        seeded = true;
                    }
                }
            }
            if (!seeded) seedDefaultCategories(db);
            ...
            if (transactions != null) {
                for (Tx t : transactions) {
                    ...
                    cv.put("note", t.note);
                    cv.put("created_at", t.createdAt);
                    db.insert("tx", null, cv);
                }
            }
            db.setTransactionSuccessful();
        } finally {
